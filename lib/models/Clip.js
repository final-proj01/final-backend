...
module.exports = class Clip {
  ...
  constructor(row) {
    ...
    this.vote_id = row.vote_id;
  }

  static async getClipsById(id) {
    const { rows } = await pool.query(`
      select * from user_clips where users_id = $1 group by id
    `, [id]);
  ...
  static async addVote({ voteUp, voteDown }, voter_id) {
    const { rows } = await pool.query(`
    insert into votes (voteUp, voteDown, voter_id) values ($1, $2, $3)
    returning *
    `, [voteUp, voteDown, voter_id]);

    return new Clip(rows[0]);
  }

  static async addComment(details, clip_id) {
    const { rows } = await pool.query(`
    insert into comments (details, clip_id) values ($1, $2)
    returning *
  `, [clip_id, details]);
    return new Clip(rows[0]);
  }
  ...
  static async getCommentsById(id) {
    ...
    as comments
    from user_clips
    left join user_comments on user_comments.comment_id = user_clips.id
    left join comments on user_comments.comment_id = comments.id
